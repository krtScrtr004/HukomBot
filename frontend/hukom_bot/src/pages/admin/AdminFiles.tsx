...
function parseDocumentAnalyticsEvent(
	eventData: string,
): AdminDocumentAnalytics | null {
	try {
...
function DocumentAnalytics({
	...
	if (loading && !data)
		return (
			<div className="rounded-sm border border-border bg-surface p-8 text-center text-sm text-text-muted">
	...
	const types = Object.entries(data.type_count)
		.filter(([, value]) => value > 0)
		.sort(([, a], [, b]) => b - a)
		.slice(0, 8);
	const typeMax = Math.max(...types.map(([, value]) => value), 1);
	return (
		<section className="space-y-4" aria-label="Document analytics">
			<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
				{[
					...
					[
						'New uploads',
						data.new_upload_count,
						'bi-cloud-arrow-up-fill',
						'text-info',
					],
					[
						'Pending review',
						data.status_count.pending,
						'bi-hourglass-split',
						'text-warning',
					],
					[
						'Processing',
						data.status_count.ongoing,
						'bi-arrow-repeat',
						'text-info',
					],
					[
						'Completed',
						data.status_count.completed,
					...
					<article
						key={String(label)}
						className="flex items-center gap-4 rounded-sm border border-border bg-surface p-4 shadow-sm"
					>
						<span
							className={`flex h-10 w-10 items-center justify-center rounded-sm bg-hover ${tone}`}
						>
				...
				<article className="rounded-sm border border-border bg-surface p-5">
					...
					<div className="mt-6 flex h-40 items-end gap-2 border-b border-border">
						{monthly.map(([month, value]) => (
							...
							>
								<div
									className="w-full rounded-t-sm bg-primary/70 group-hover:bg-primary"
									style={{
										height: `${Math.max((value / monthlyMax) * 100, 3)}%`,
									}}
									title={`${month}: ${value}`}
								/>
						...
						{statuses.map(([status, value]) => (
							<div key={status}>
								...
								<div className="h-2 rounded-full bg-hover">
									<div
										className={`h-full rounded-full ${status === 'completed' ? 'bg-success' : status === 'pending' ? 'bg-warning' : status === 'failed' || status === 'rejected' ? 'bg-danger' : 'bg-info'}`}
										style={{
											width: `${Math.max((value / Math.max(data.total_count, 1)) * 100, value ? 4 : 0)}%`,
										}}
									/>
								</div>
							</div>
			...
			<article className="rounded-sm border border-border bg-surface p-5">
				...
				<p className="mt-1 text-xs text-text-secondary">
					Top legal document categories
				</p>
				<div className="mt-5 grid gap-x-8 gap-y-3 md:grid-cols-2">
					{types.map(([type, value]) => (
						<div
							key={type}
							className="grid grid-cols-[minmax(7rem,12rem)_1fr_2rem] items-center gap-3 text-xs"
						>
			...
			{data.most_upload_user.length > 0 && (
				<article className="rounded-sm border border-border bg-surface p-5">
					<div className="flex items-center justify-between">
						<div>
							<h2 className="font-semibold">Top uploaders</h2>
							<p className="mt-1 text-xs text-text-secondary">
								Users with the most uploaded documents
							</p>
						</div>
						<i
							...
							.map((user, index) => (
								...
								>
									...
									<strong className="text-sm">
										{user.upload_count.toLocaleString()}
									</strong>
...
export default function AdminFilesPage() {
	const { state, dispatch, fetchDocuments, fetchDocumentAnalytics } =
		useAdmin();
	...
	const [selectedDocument, setSelectedDocument] =
		useState<AdminDocumentListItem | null>(null);
	...
	useEffect(() => {
		...
		const open = () => {
			...
			stream = new EventSource(`${EVENT_BASE_URL_V1}/admin/documents`, {
	...
	const handleSingleDelete = async () => {
		...
		try {
			...
			setSelectedIds((prev) =>
				prev.filter((id) => id !== singleDeleteTarget.id),
			);
			void fetchDocuments();
			void fetchDocumentAnalytics();
		} catch (error) {
	...
	const handleBulkDelete = async () => {
		...
		try {
			...
			showToast(
				`${selectedIds.length} document(s) deleted successfully`,
				'success',
			);
			setSelectedIds([]);
			...
			render: (row) => (
				<div className="flex items-center gap-3 min-w-50">
					<div className="flex h-9 w-9 items-center justify-center rounded-lg bg-surface-muted border border-border shrink-0">
						<i
							className={`bi ${getFileIcon(row.original_file_name)} text-lg`}
						/>
					</div>
	...
	return (
		<div className="space-y-4">
				...
				<div>
					<div className="flex items-center gap-2">
						<h1 className="text-2xl font-bold tracking-tight text-text-primary flex items-center gap-2">
							<i className="bi bi-folder-symlink text-primary" />{' '}
					...
					<p className="text-sm text-text-secondary mt-1">
						Browse uploaded legal documents, review pending files,
						and track chunking progress.
					</p>
				</div>
			</header>
			...
			<DocumentAnalytics
				...
				loading={documentAnalytics.loading}
			/>
			...
			<main className="space-y-4">
				<div className="rounded-sm border border-border bg-surface p-5 shadow-xs space-y-4">
					...
					<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
						...
						<div className="flex items-center gap-2 shrink-0">
							...
							{documents.items.length > 0 && (
								...
								>
									<i
										className={`bi ${allCurrentSelected ? 'bi-check-square-fill text-primary' : 'bi-square'}`}
									/>
									<span>
										{allCurrentSelected
											? 'Deselect Page'
					...
					{documents.uploaderIdFilter ? (
						<div className="flex items-center gap-2 text-xs text-text-secondary bg-surface-muted border border-border rounded-md px-3 py-1.5 w-fit">
							...
							<span>
								...
								<strong>{documents.uploaderIdFilter}</strong>
							</span>
							<button
								type="button"
								onClick={() => {
					...
					<DataTable
						...
						emptyMessage={emptyMessageForFilter(
							documents.statusFilter,
						)}
					...
					<div className="flex items-center justify-between pt-3 border-t border-border">
						...
						<div className="flex items-center gap-2">
							<button
								type="button"
								disabled={
									documents.offset === 0 || documents.loading
								}
								onClick={() =>
									dispatch({
										type: 'SET_DOCS_OFFSET',
							...
							<button
								type="button"
								disabled={
									!documents.hasMore || documents.loading
								}
								onClick={() =>
									dispatch({
										type: 'SET_DOCS_OFFSET',
