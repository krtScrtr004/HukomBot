...
from backend.app.repository.case_analysis_session_repository import CaseAnalysisSessionRepository
from backend.app.repository.case_analysis_version_repository import CaseAnalysisVersionRepository
from backend.app.repository.case_analysis_version_fact_repository import CaseAnalysisVersionFactRepository
from backend.app.repository.case_fact_repository import CaseFactRepository
from backend.app.repository.case_fact_version_repository import CaseFactVersionRepository
...
def get_case_fact_version_repository(db: Database = Depends(get_db)) -> CaseFactVersionRepository:
    return CaseFactVersionRepository(db=db)


def get_case_analysis_session_repository(db: Database = Depends(get_db)) -> CaseAnalysisSessionRepository:
    return CaseAnalysisSessionRepository(db=db)


def get_case_analysis_version_repository(db: Database = Depends(get_db)) -> CaseAnalysisVersionRepository:
    return CaseAnalysisVersionRepository(db=db)


def get_case_analysis_version_fact_repository(db: Database = Depends(get_db)) -> CaseAnalysisVersionFactRepository:
...
def get_case_analysis_service(
    ...
    case_fact_version_repo: CaseFactVersionRepository = Depends(get_case_fact_version_repository),
    case_analysis_session_repo: CaseAnalysisSessionRepository = Depends(get_case_analysis_session_repository),
    case_analysis_version_repo: CaseAnalysisVersionRepository = Depends(get_case_analysis_version_repository),
    case_analysis_version_fact_repo: CaseAnalysisVersionFactRepository = Depends(get_case_analysis_version_fact_repository),
